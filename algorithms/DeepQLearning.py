...
# Email: bilel.khlaifia@icloud.com

import sys, cv2
import nvidia_smi
from network.agent import AutopilotAgent
...
def DeepQLearning(cfg, env_process, env_folder):
    ...
    if cfg.mode == 'train':
        ...
        if algorithm_cfg.distributed_algo == 'GlobalLearningGlobalUpdate-MA':
            ...
            global_agent = AutopilotAgent(algorithm_cfg, client, name='DQN', vehicle_name='global')
            ReplayMemory = Memory(algorithm_cfg.buffer_len)
            target_agent = AutopilotAgent(algorithm_cfg, client, name='Target', vehicle_name='global')
        ...
        for drone in range(cfg.num_agents):
            ...
            agent[name_agent] = AutopilotAgent(algorithm_cfg, client, name='DQN', vehicle_name=name_agent)

            if algorithm_cfg.distributed_algo != 'GlobalLearningGlobalUpdate-MA':
                ReplayMemory[name_agent] = Memory(algorithm_cfg.buffer_len)
                target_agent[name_agent] = AutopilotAgent(algorithm_cfg, client, name='Target', vehicle_name=name_agent)
            current_state[name_agent] = agent[name_agent].get_state()

    elif cfg.mode == 'infer':
        name_agent = 'drone0'
        name_agent_list.append(name_agent)
        agent[name_agent] = AutopilotAgent(algorithm_cfg, client, name=name_agent + 'DQN', vehicle_name=name_agent)
